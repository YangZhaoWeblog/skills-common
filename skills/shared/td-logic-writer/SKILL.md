...
把混乱材料整理成可理解、可复用的判断或解释。以尽可能少的阅读成本让目标读者正确理解或使用，受准确性、必要关系和关键边界约束。个人决策让未来的自己恢复判断；解释型博文让陌生读者独立理解。
...
使用情境、规则依据、schema 用法和完整正反短例见 [current-content-integrity.md](references/current-content-integrity.md)。只做快速审查时先读 [review-checklist.md](references/review-checklist.md)，再按问题读取相关 reference。
...
也适用于用户要求成文的解释型博文：围绕明确问题讲清概念或机制，让陌生读者独立理解；不强制转成判断标准文。交互式深度教学仍走学习链路。
...
拿不准时看任务：帮助未来的自己判断、选择、负责、止损，或为陌生读者写清一个概念与机制，都可使用本 skill；其他任务转给对应链路。
...
| 解释型博文 | 必要背景、概念和机制之间的关系，以及适用边界 |
| 项目认知 | 责任边界、关键关系与负责人的判断点 |
| 判断标准 | 可复用的判断条件、反例与动作 |
| 证据型决策 | 默认选择、改变选择的条件、候选结论与停止条件 |
| 个人系统/决策文档 | 问题与理由、约束与价值选择、判断规则、动作边界与调整条件 |

具体骨架、证据位置和压缩步骤见 [writing-workflow.md](references/writing-workflow.md)。

## Core Rules

- 先写文章契约：读者、使用情境、真实问题，以及需要理解的关系或形成的判断与适用边界；允许先探讨本质，不要求预设默认选择。
...
- 结构先于句子：先检查话题与使用场景切换，修标题链、章节职责和阅读依赖，再压缩句子；不能靠缩句掩盖整体割裂。
...
- 每节围绕一个主要的理解或判断任务；连续解释按阅读依赖组织，同一结论的重复说法不另拆章节。
- 一条关键判断只设一个主载体。其他位置仅保留当前理解或使用必需的条件、例外、证据或动作，并通过删除检验。
...
- 图或表已经给出结论，正文保留理解所需的定义、关系、理由、例外或后果，不逐行复述。
- 写作、压缩和 review 统一按 [current-content-integrity.md](references/current-content-integrity.md) 的最小充分解释与删除检验执行：直接给判据，补必要关系，懂了停；复杂推导保留充分因果与边界，不设字数或句数硬限。
- 保留事实不等于保留原句；压缩时不得把“本次证据 / 某研究 / 已核实范围”扩大成“全部现有 / 普遍事实”，也不得把“不能支持”写强成“证明”或“排除”。
- 用户要求去重或压缩时，必须实际合并、移位或删除重复载体；只换标题和语序不算完成。
- 不写默认总结。仅在读者确需综合判断或集中执行清单、且通过删除检验时保留。
