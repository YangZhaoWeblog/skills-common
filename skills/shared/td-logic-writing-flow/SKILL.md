...
这是路由器，不是另一个 writer。先识别文档类型和阅读组织方式，再进入对应链路。
...
- **阅读组织方式：** 按任务选择推导、比较、逐层理解、案例验证或并列查阅等方式，不固定四选一。
...
- `gate_id` 标识一次修改任务，关联目标身份与候选版本；生命周期、记录恢复和暂停条件以 [review-gate.md](references/review-gate.md) 为准。
...
- 已完成任务后的错别字、局部润色和用户限定的快速小改，按当前影响判断是否豁免；同一未完成任务不得通过拆分、改名或换会话绕过待解决问题。单次或累计影响判断、结构、事实或视觉层级时，必须独立 Review。
- 复审次数用于追踪进展，不是文章的终身额度；修复后可继续复审，最终候选仍须通过独立 Review。
